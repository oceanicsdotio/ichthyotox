...
fig.subplots_adjust(top=1.0-vpadding, bottom=vpadding+0.1, left=hpadding, right=1.0-hpadding/2.0)
ax = []
...
#######################################
print "Reading Positions (A)" # experiment A
data = np.loadtxt('../100/fish_position.dat', unpack=True)
for ii in range(0, nfish):
    xcol = ii*dwidth + 2
    xpos[ii,:] = data[xcol,:]
print "Calculating Mean Suitability (A)"
suitability = 0.5*(1.0 + sin( 2.0 *pi * (xpos[:,:]-125.0) / 500.0 ))
suit_avg = np.mean(suitability[:,:], axis=0)
for ii in range(0, 2*window+1):
    suit_smoothed[start+window:end-window] = suit_smoothed[start+window:end-window] + suit_avg[start+ii:end+ii-2.0*window]/(2.0*window+1.0)
ax[0].plot(time[start+window:end-window], suit_smoothed[start+window:end-window], linestyle='-', linewidth=1, color='white', aa=True, label='Control (A)',zorder=3)

######################################## experiment B
print "Reading Positions (B)"
data = np.loadtxt('../101/fish_position.dat', unpack=True)
for ii in range(0, nfish):
    xcol = ii*dwidth + 2
    xpos[ii,:] = data[xcol,:]
print "Calculating Mean Suitability (B)"
suitability = 0.5*(1.0 + sin( 2.0 *pi * (xpos[:,:]-125.0) / 500.0 ))
suit_avg = np.mean(suitability[:,:], axis=0)
suit_smoothed = zeros(end-start+1)
for ii in range(0, 2*window+1):
    suit_smoothed[start+window:end-window] = suit_smoothed[start+window:end-window] + suit_avg[start+ii:end+ii-2.0*window]/(2.0*window+1.0)
ax[0].plot(time[start+window:end-window], suit_smoothed[start+window:end-window], linestyle='-', linewidth=1, color='red', aa=True, label='Formation (B)',zorder=4)
######################################## experiment C
print "Reading Positions (C)" 
data = np.loadtxt('../102/fish_position.dat', unpack=True)
for ii in range(0, nfish):
    xcol = ii*dwidth + 2
    xpos[ii,:] = data[xcol,:]
print "Calculating Mean Suitability (C)"
suitability = 0.5*(1.0 + sin( 2.0 *pi * (xpos[:,:]-125.0) / 500.0 ))
suit_smoothed = zeros(end-start+1)

suit_avg_surface = np.mean(suitability[0:nfish/2-1,:], axis=0)
for ii in range(0, 2*window+1):
    suit_smoothed_s[start+window:end-window] = suit_smoothed_s[start+window:end-window] + suit_avg_surface[start+ii:end+ii-2.0*window]/(2.0*window+1.0)

suit_avg_bottom = np.mean(suitability[nfish/2:nfish-1,:], axis=0)
for ii in range(0, 2*window+1):
    suit_smoothed_b[start+window:end-window] = suit_smoothed_b[start+window:end-window] + suit_avg_bottom[start+ii:end+ii-2.0*window]/(2.0*window+1.0)

ax[0].plot(time[start+window:end-window], suit_smoothed_s[start+window:end-window], linestyle=':', linewidth=1, color='green', aa=True)
ax[0].plot(time[start+window:end-window], suit_smoothed_b[start+window:end-window], linestyle='-', linewidth=1, color='green', aa=True, label='Intensification (C)')
######################################## experiment D
print "Reading Positions (D)"
data = np.loadtxt('../103/fish_position.dat', unpack=True)
for ii in range(0, nfish):
    xcol = ii*dwidth + 2
    xpos[ii,:] = data[xcol,:]
print "Calculating Mean Suitability (D)"
suitability = 0.5*(1.0 + sin( 2.0 *pi * (xpos[:,:]-125.0) / 500.0 ))
suit_smoothed = zeros(end-start+1)
suit_smoothed_s = zeros(end-start+1)
suit_smoothed_b = zeros(end-start+1)
...
ax[0].plot(time[start+window:end-window], suit_smoothed_s[start+window:end-window], linestyle=':', linewidth=1, color='blue', aa=True)
ax[0].plot(time[start+window:end-window], suit_smoothed_b[start+window:end-window], linestyle='-', linewidth=1, color='blue', aa=True, label='Decline (D)')
#######################################

ax[0].plot((time[start], time[end]),(0.5, 0.5), linestyle='--', linewidth=1, color='white', aa=True, zorder = 5)
...
#plt.show()
plt.savefig('./fish_suitability_screen.tiff', facecolor=bg_color, edgecolor='none', dpi=default_dpi)
