...
rects_a = ax[0].bar(np.arange(nphases), msd_value_a[:,0], width, color='white', edgecolor='none', label='Control (A)')
...
rects_c = ax[0].bar(np.arange(nphases)+2.5*width, msd_value_c[:,1], width/2, color=[0.0,1.0,0.0,0.3], edgecolor='none', label='Intensification, demersal')
...
rects_d = ax[0].bar(np.arange(nphases)+3.5*width, msd_value_d[:,1], width/2, color=[0.0,0.0,1.0,0.3], edgecolor='none', label='Decline, demersal')
...
plt.savefig('./fish_diffusivity_screen.tiff', facecolor=bg_color, edgecolor='none', dpi=default_dpi)
