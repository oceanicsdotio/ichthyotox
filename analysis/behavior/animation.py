...
if __name__ == "__main__":
    # load data
    experiment = "/Users/keeney/Documents/Projects/2020_2017_Ichthyotox/data/100"
    ini = open(f'{experiment}/fish_ini.dat', 'r')
    nparticles = int(str.strip(ini.readline()))
    time = np.loadtxt(f'{experiment}/fish_position.dat', usecols=[0], unpack=True)
    start = 0
    end = len(time) - 1
    datawidth = 4
    span = end-start+1
    ...
    for ii in range(0, nparticles):
        ...
        ycol = ii*datawidth + 3
        zcol = ii*datawidth + 4
        fpsx[ii,:], fpsy[ii,:] = np.loadtxt(f'{experiment}/fish_position.dat', usecols=(xcol, ycol), unpack=True)

    # Calculate Domain Data
    vert_x, vert_y = np.loadtxt(f'{experiment}/mesh_node.dat', skiprows=1, usecols=(1,2), unpack=True)
    xmax = max(vert_x[:])
    ymax = max(vert_y[:])
    xmin = min(vert_x[:])
    ymin = min(vert_y[:])
    ...
    ax.xaxis.set_minor_locator(MultipleLocator(xmax))
    ax.xaxis.grid(False)

    ax.set_ylabel(r'northing (m)')
    ax.set_ylim(ymin,ymax)
    ax.yaxis.set_major_locator(MultipleLocator(ymax))
    ax.yaxis.set_minor_locator(MultipleLocator(ymax))
    ax.yaxis.grid(False)
