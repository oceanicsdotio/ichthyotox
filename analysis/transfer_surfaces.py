...
fig = plt.figure(facecolor=bg_color, figsize=(marginWidth, 2.*marginWidth/3.)) #Change this
...
ax[0].plot_surface(irradiance, log(rgr), abs(fixation), color=bg_color, edgecolor=overlay_color, linewidth=1, antialiased=True, shade=False)
...
ax[1].plot_surface(temperature, log(rgr), abs(synthesis), color=bg_color, edgecolor=overlay_color, linewidth=1, antialiased=True, shade=False)
...
# RESPIRATION
tp2 = 0.286*exp(.05*temperature - 0.15)
...
ax[2].plot_surface(temperature, log(rgr), abs(respiration), color=bg_color, edgecolor=overlay_color, linewidth=1, antialiased=True, shade=False)

# EXCRETION
excretion = .1*(0.004*carbsgr + 0.05*progr)*tp2gr
ax[3].plot_surface(temperature, log(rgr), abs(excretion), color=bg_color, edgecolor=overlay_color, linewidth=1, antialiased=True, shade=False)
...
for ii in range(0,4):
    ...
    ax[ii].xaxis.set_major_locator(LinearLocator(2)); ax[ii].xaxis.set_major_formatter(FormatStrFormatter('%.0f'))
    ax[ii].xaxis.grid(False); ax[ii].xaxis.set_rotate_label(False); ax[ii].w_xaxis.set_pane_color(bg_color)

    ax[ii].set_ylabel(sublabels[ii][1]); ax[ii].set_ylim()
    ax[ii].yaxis.set_major_locator(LinearLocator(2)); ax[ii].yaxis.set_major_formatter(FormatStrFormatter('%.0f'))
    ax[ii].yaxis.grid(False); ax[ii].yaxis.set_rotate_label(False); ax[ii].w_yaxis.set_pane_color(bg_color)

    ax[ii].set_zlabel(sublabels[ii][2])
    ax[ii].autoscale(enable=True, axis='z', tight=True)
    #ax[ii].set_zlim(0.0,)
    ax[ii].zaxis.set_major_locator(LinearLocator(2)); ax[ii].zaxis.set_major_formatter(FormatStrFormatter('%.1f'))
    ax[ii].zaxis.grid(False); ax[ii].zaxis.label.set_rotation(360.0); ax[ii].w_zaxis.set_pane_color(bg_color)
...
#fig.tight_layout()
plt.savefig('./transfer_surfaces_screen.tiff', facecolor=bg_color, edgecolor='none', dpi=default_dpi)
#plt.show()