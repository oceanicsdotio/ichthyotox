...
subroutine hunt(sigma_nodes, KB, sigma_particle, jlo) ! Z, KB, self%ZP(ii), NZR
  ...
  ascnd = (sigma_nodes(KB) .gt. sigma_nodes(1)) ! true if bottom sigma layer greater than first sigma layer
  if ((jlo .le. 0) .or. (jlo .gt. KB)) then
     jlo = 0
     jhi = KB + 1
     goto 3
  endif
  ...
  if ((sigma_particle .ge. sigma_nodes(jlo)) .eqv. ascnd) then
1    jhi = jlo + inc
     if(jhi .gt. KB)then
        jhi=n+1
     else if ((sigma_particle .ge. sigma_nodes(jhi)) .eqv. ascnd) then
        jlo = jhi
        inc = inc + inc
        goto 1
     end if
...
2    jlo = jhi - inc
     if (jlo.lt.1) then
        jlo=0
     else if ((sigma_particle .lt. sigma_nodes(jlo)) .eqv. ascnd) then
        jhi = jlo
        inc = inc + inc
        go to 2
     end if
...
3 if (jhi-jlo .eq. 1) then
     if (sigma_particle .eq. sigma_nodes(KB)) jlo = KB - 1
     if (sigma_particle .eq. sigma_nodes(1)) jlo = 1
     return
  end if
  jm = (jhi + jlo)/2
  if (sigma_particle .ge. sigma_nodes(jm) .eqv. ascnd) then
     jlo = jm
  else
     jhi = jm
  end if
