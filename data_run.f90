...
! Input Parameters Which Control the Model Run
  ...
  IPT = 71
  if (trim(INFOFILE) .ne. "screen") then
  ...
  ii = len_trim(GEOAREA)
  if(GEOAREA(ii:ii) .eq. "/") GEOAREA(ii:ii) = " "
  ...
  ii = len_trim(INPDIR)
  if(INPDIR(ii:ii) .eq. "/") INPDIR(ii:ii) = " "
  ...
  ii = len_trim(LAGINI)
  if (LAGINI(ii:ii) .eq. "/") LAGINI(ii:ii) = " "
  ...
  ii = len_trim(OUTDIR)
  if (OUTDIR(ii:ii) .eq. "/") OUTDIR(ii:ii) = " "
