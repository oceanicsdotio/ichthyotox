...
! DETERMINE NBE(i=1:n,j=1:3): INDEX OF 1 to 3 NEIGHBORING ELEMENTS
   ...
   DO ii = 1, N
     ...
     DO J1 = 1, CELLCNT(N1) 
     DO J2 = 1, CELLCNT(N2) 
       IF ((CELLS(N1, J1) .eq. CELLS(N2, J2)) .AND. CELLS(N1,J1) .ne. ii) NBE(ii, 3) = CELLS(N1, J1)
     END DO
     END DO
     DO J2 = 1, CELLCNT(N2) 
     DO J3 = 1, CELLCNT(N3) 
       IF ((CELLS(N2, J2) .eq. CELLS(N3, J3)) .AND. CELLS(N2, J2) .ne. ii) NBE(ii,1) = CELLS(N2, J2)
     END DO
     END DO
     DO J1 = 1, CELLCNT(N1) 
     DO J3 = 1, CELLCNT(N3) 
       IF((CELLS(N1, J1) .eq. CELLS(N3,J3)) .AND. CELLS(N1,J1) .ne. ii) NBE(ii, 2) = CELLS(N3, J3)
     END DO
     END DO
   END DO
...
! if element on boundary set isbce(i)=1 and isonb(j)=1 for boundary nodes j
   do ii = 1, N 
     if ( MIN(NBE(ii, 1), NBE(ii, 2), NBE(ii, 3)) .eq. 0 ) then
       ISBCE(ii) = 1  ! element on boundary
       if (NBE(ii, 1) .eq. 0) then 
         ISONB(NV(ii, 2)) = 1 ; ISONB(NV(ii, 3)) = 1
       end if
       if (NBE(ii,2) .eq. 0) then
         ISONB(NV(ii, 1)) = 1 ; ISONB(NV(ii, 3)) = 1
       end if
       if (NBE(ii,3) .eq. 0) then
...
! Determine max number of surrounding elements
   ...
   element_loop: do ii = 1, M
     NCNT = 0
     node_loop: do jj = 1, N
       if ( float(NV(jj, 1) - ii) * float(NV(jj, 2) - ii) * float(NV(jj, 3) - ii) .eq. 0.0_SP ) NCNT = NCNT + 1
     end do node_loop
...
! determine nbvt - index (1,2, or 3) of node i in neighboring element
   ...
   do ii = 1, M
     NCNT = 0
     do jj = 1, N
        if ( float(NV(jj, 1) - ii) * float(NV(jj, 2) - ii) * float(NV(jj, 3) - ii) .eq. 0.0_SP) then
         NCNT = NCNT+1
         NBVE(ii, NCNT) = jj
         if ((NV(jj, 1) - ii) .eq. 0) NBVT(ii, NCNT) = 1
         if ((NV(jj, 2) - ii) .eq. 0) NBVT(ii, NCNT) = 2
         if ((NV(jj, 3) - ii) .eq. 0) NBVT(ii, NCNT) = 3
       end if
     end do
     NTVE(ii) = NCNT
   end do
   ...
   do ii = 1, M
     if (ISONB(ii) .eq. 0) then
       ...
       do jj = 2, NTVE(ii) + 1
         ...
         ll=NB_TMP(jj, 1)
         IF ((NV(ll, 1) - ii) .eq. 0) NB_TMP(jj, 2) = 1
         IF ((NV(ll, 2) - ii) .eq. 0) NB_TMP(jj, 2) = 2
         IF ((NV(ll, 3) - ii) .eq. 0) NB_TMP(jj, 2) = 3
       end do
       ...
       NTMP = NTVE(ii) + 1
       if (NBVE(ii, 1) .ne. NBVE(ii, NTMP)) then
         print*, ii,'nbve(ii) not correct!!'
         stop
       end if
       if (NBVT(ii,1) .ne. NBVT(ii, NTMP)) then
       ...
       do jj = 1, NTVE(ii)
         ll = NBVT(ii, jj)
         if (NBE(NBVE(ii, jj), ll + 2 - int((ll + 2)/4)*3) .eq. 0) then
           JJB = JJB + 1
...
!   enddo
   ...
   do ii = 1, n
     if (isbce(ii) .gt. 1) then
       do jj = 1, 4
         a1u(ii, jj) = 0.0_SP
         a2u(ii, jj) = 0.0_SP
       end do
     else if (isbce(ii) .eq. 1) then
       do jj = 1, 3
         if (nbe(ii, jj) .eq. 0) ll = jj
       end do
