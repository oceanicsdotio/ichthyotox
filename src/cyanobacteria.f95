module Cyanobacteria
    ...
    type, public, extends(Agent) :: CyanobacteriaAgent
        ...
        real(sp) :: &
            & mclrProductionRate = 0.0_sp, &
            & mclrExcretionRate = 0.0_sp
        ...
        real(sp), allocatable, dimension(:), public :: &
            & carbohydrate, & ! mass of carbohydrate ballast, grams
            & protein, & ! mass of buoyant cellular material, grams
            & microcystin ! mass of cellular toxins, grams

    contains
        ! user interface subroutines
        procedure, public :: init => initialize ! read position and count, allocate state variables
        procedure, public :: writeState => writeState ! write internal state variables
        procedure, public :: movement => verticalMovement ! subroutine that updates position of colony particles
        procedure, public :: random => random_walk ! vertical random walk for cyanobacteria

        ! private utility procedures
        procedure, private :: tempLimit ! scaling coefficients
        procedure, private :: tempFunction => temperatureFunction ! scaling coefficients

        ! movement function call aliases
        procedure, private :: velocity => stokesVelocity ! particle vertical velocities
        procedure, private :: density => algaeDensity ! colony densities
        procedure, private :: viscosity => dynamicViscosity ! viscosity at particle locations

        ! mass transfer function call aliases
        procedure, private :: fixation => carbonFixation ! subroutine adds mass to carbohydrate ballast
        procedure, private :: synthesis => carbonSynthesis ! subroutine moves mass from carbohydrate to protein
        procedure, private :: excretion => carbonExcretion ! subroutine subtracts mass from protein
        procedure, private :: respiration => carbonRespiration ! subroutine subtracts mass from carbohydrate
    ...
    public :: randomRadius, equilibriumRatio, waterDensity, writeInitialState
...
contains
    ...
    subroutine initialize(self)
        ...
        integer :: ii, indexMatch, fid=100
        character(len=50) :: filename
        logical :: fexist
        allocate(self%radius(self%count)); self%radius = 0.0_sp
        allocate(self%zp, self%carbohydrate, self%protein, self%microcystin, source=self%radius)
        open (unit=fid, file=trim(folderprefix)//'cyanobacteria.txt', form='formatted')
        read (fid, "(I6)") self%count
        do ii = 1, self%count
            read (fid, "(5F20.6)") &
                & self%zp(ii), self%radius(ii), self%carbohydrate(ii), self%protein(ii), self%microcystin(ii)
        end do
        close (fid)
    end subroutine


    subroutine verticalMovement(self)
        ! update position due to buoyant movement
        use simulation, only: domain
        use variables, only: dti, KBM1 ! integration step
        use variables, only: A_RK, B_RK, MSTAGE ! runge-kutta integration parameters

        class(CyanobacteriaAgent), intent(inout) :: self
        integer :: ii
        real(sp) :: mcoef, update_coef
        ! initial values
        real(sp), dimension(self%count) :: idz, ini_position, ini_sigma, ini_microcystin, ini_carbohydrate, ini_protein 
        ! stage fcn evaluations
        real(sp), dimension(self%count, 0:MSTAGE) :: chi_dissolved, chi_position, chi_carbohydrate, chi_protein, chi_microcystin 
        real(sp), dimension(self%count) :: synthesis, excretion, respiration, fixation, mc_excretion, mc_production, calc_array
        chi_position = 0.0_sp; 
        chi_carbohydrate = 0.0_sp; 
        chi_protein = 0.0_sp; 
        chi_microcystin = abs(0.0_sp)

        ! save initial values
        ini_carbohydrate(:) = self%carbohydrate(:)
        ini_protein(:) = self%protein(:)
        ini_microcystin(:) = self%microcystin(:)
        ini_position(:) = self%zp(:)

        do ii = 1, MSTAGE
            ! get new stage values
            update_coef = A_RK(ii)*dti
            self%zp(:) = ini_position(:) + update_coef * chi_position(:, ii - 1)      ! new stage position
            self%carbohydrate(:) = ini_carbohydrate(:) + update_coef * chi_carbohydrate(:, ii - 1)  ! new carb stage value
            self%protein(:) = ini_protein(:) + update_coef * chi_protein(:, ii - 1)       ! new protein stage value
            self%microcystin(:) = ini_microcystin(:) + update_coef * chi_microcystin(:, ii - 1)   ! new toxin stage value

            ! enforce strict limits on state variables
            self%carbohydrate(:) = max(self%carbohydrate, 0.0_sp) ! non-negative
            self%protein(:) = max(self%protein, 0.0_sp) ! non-negative
            self%microcystin = max(self%microcystin, 0.0_sp) ! non-negative
            self%zp = min(self%zp, self%ep) ! stop at surface
            self%zp = max(self%zp, self%hp) ! stop at sediment
        ...
        do ii = 1, MSTAGE ! add weighted stages to initial values
            ...
            self%zp(:) = self%zp(:) + chi_position(:, ii)*B_RK(ii)*dti ! update depth
            ...
            where (self%zp(:) > self%ep(:))
                self%zp(:) = self%ep(:)
                else where (self%zp(:) < self%hp(:))
                self%zp(:) = self%hp(:)
            end where

            ! update dissolved toxin  at particle position for each stage
            self%zp(:) = self%sigma(self%zp(:)) ! update sigma
        ...
        self%carbohydrate(:) = max(self%carbohydrate(:), 0.0_sp)
        self%protein(:) = max(self%protein(:), 0.0_sp)
        where (self%microcystin(:) < 0.0_sp) self%microcystin(:) = 0.0_sp
        self%zp(:) = min(self%zp(:), self%ep(:)) ! collect at surface, mixing during random walk
        self%zp(:) = max(self%zp(:), self%hp(:)) ! collect at sediment, mixing during random walk
        self%layer(:) = self%zlocate(self%zp(:)) ! update layer
    ...
    subroutine random_walk(self, noise)
        ! vertical random walk
        use variables, only: dti, dtrw
        class(CyanobacteriaAgent), intent(inout) :: self
        real(sp), dimension(self%count) :: noise
        real(sp), dimension(self%count) :: wdiff, kzp, dkzp ! diffusivity and first derivative at particle positions
        integer :: substeps
        real(sp), parameter :: variance = 1.0
        ...
        do substeps = 1, int(dti/dtrw)
            ...
            self%zp(:) = self%zp(:) + wdiff(:)
            self%zp(:) = min(self%zp(:), self%ep(:)) ! stop at surface during integration
            self%zp(:) = max(self%zp(:), self%hp(:)) ! stop at sediment during integration
            self%layer(:) = self%zlocate(self%zp(:))
        end do
    end subroutine

    subroutine writeState(self, fid, time)
        ...
        integer :: ii
        write(fid, "(1F10.2,9000(3F20.3))") time, & 
            & (self%carbohydrate(ii), self%protein(ii), &
            & self%microcystin(ii), ii=1,self%count)
    end subroutine
    ...
    pure elemental function fixation_coefficient(biomass, irradiance, z) result(fixation)
        real(sp), intent(in) :: biomass, irradiance, z
        real(sp) :: &
            & irradRatio, & ! array of particle specific actual:optimal light ratios
            & avg_self_shade, & 
            & shaded, &
            & fixation
        ! average area under self shading curve from zero to self biomass
        avg_self_shade = (exp(-lightExtinctionBiomass*biomass) - 1.0)/(-lightExtinctionBiomass*biomass)
        shaded = irradiance * avg_self_shade
        shaded = shaded * exp(z*lightAttenuationWater) ! irradiance after attenuation
        irradRatio = shaded/irradOpt
        fixation = (2.0 + fixationBeta)*irradRatio/(irradRatio**2.0 + &
            & fixationBeta*irradRatio + 1.0) * fixationMax
    end function

    function carbonFixation(self) result(fixation)
        ! Updates carbohydrate ballast state variable due to fixation (alias is "fixation")
        use simulation, only: domain
        class(CyanobacteriaAgent), intent(in) :: self
        real(sp), dimension(self%count) :: &
            & proxy_depth, & ! copy array for updating depth by recursive binary partitioning
            & biomass, &
            & irradiance, &
            & fixation
        integer :: ii
        integer, dimension(self%count) :: indices

        indices(:) = [(ii, ii=1,self%count)] ! particle indices
        ...
        fixation = fixation_coefficient(biomass, irradiance, self%zp) * &
            & self%protein*(1.0 - vesicleFrac) * &
            & (carbonRatioMax - self%carbohydrate/self%protein)/carbonRatioMax 
    end function

    pure function carbonSynthesis(self) result(synthesis)
        ! updates carbohydrate and protein state variables due to synthesis transport (alias is "synthesis")
        class(CyanobacteriaAgent), intent(in) :: self
        real(sp), dimension(self%count) :: synthesis
        synthesis = self%carbohydrate * synthesisMax * self%tempLimit()
    end function

    pure function carbonExcretion(self) result(excretion)
    ...
    pure function carbonRespiration(self) result(respiration)
        ...
        class(CyanobacteriaAgent), intent(in) :: self
        real(sp), dimension(self%count) :: respiration
    ...
    pure function tempLimit(self) result(limit)
        ...
        class(CyanobacteriaAgent), intent(in) :: self
        real(sp), dimension(self%count) :: limit ! array of output coefficients for each particle
        limit = (self%temp / tempOpt * (  ((self%temp - tempLethal)/(tempOpt - tempLethal))** &
            & ( (tempRef - tempOpt) / tempOpt )  ))**(4.0)
    end function

    pure function temperatureFunction(self) result(coefficient)
        ! returns array of scaling coefficents for biometric fcns
        class(CyanobacteriaAgent), intent(in) :: self
        real(sp), dimension(self%count) :: coefficient ! array of output coefficients for each particles
    ...
    pure function colony_microcystinProduction(self) result(production)
        ...
        class(CyanobacteriaAgent), intent(in) :: self
        real(sp), dimension(self%count) :: production ! array of microcystin production for colony particles
    ...
    pure function stokesVelocity(self) result (velocity)
    ...
    pure function algaeDensity(self) result(density)
        ! colony density including contibutions of mucus and gas vacuoles
        class(CyanobacteriaAgent), intent(in) :: self
        real(sp), dimension(self%count) :: density ! output array of overall colony density
        density = algae_density(self%carbohydrate, self%protein, self%rho)
    end function

    pure function dynamicViscosity(self) result(viscosity)
        ! returns array of dynamic viscosity values at particle locations
        class(CyanobacteriaAgent), intent(in) :: self
        logical :: simple;
        real(sp), dimension(self%count) :: viscosity
        simple = .true.
        if (simple) then
    ...
    pure elemental function waterDensity(temperature, salinity) result(density)
    ...
    pure function randomRadius(random_component) result(radius)
        real(sp), intent(in) :: random_component
        real(sp) :: radius
        radius = colonyBaseRadius + 5.0_SP*10.0_SP**(-6.0_SP)*random_component
    end function

    pure elemental function equilibriumDensity(water) result(density)
        ! returns equilibrium density of colony
        real(sp), intent(in) :: water
        real(sp) :: density
        density = ((water - (1.0_SP - cellFrac)*(water + 0.7_SP)) / &
            & cellFrac - vesicleFrac*vesicleDensity)/(1.0_SP - vesicleFrac)
    end function

    pure elemental function equilibriumRatio(water) result(ratio)
        ! returns equilibrium carbon ratio of colony
        real(sp), intent(in) :: water
        real(sp) :: ratio
        ratio = log(1.0_SP - (equilibriumDensity(water) - densityMin)/(densityMax - densityMin)) / &
            & (-cellDensityCoefficient)
    end function

    subroutine writeInitialState(file, count, biomass, toxin, density, rng)
        use random, only : Gaussian_Random_Number
        integer, intent(in) :: count
        type(Gaussian_Random_Number), intent(inout) :: rng
        real(sp), intent(in) :: biomass, toxin, density
        integer :: fid = 100, ii
        character(len = * ), intent(in) :: file
        real(sp) :: z, radius, protein, carbohydrate
        open(unit = fid, file=file, status = 'replace')
        print *, 'Saving cyanobacteria state...'
        write(fid, "(I6)") count
        do ii = 1, count
            z = -1.0*abs(rng%uniform()*5.0)
            radius = colonyBaseRadius + 5.0_SP*10.0_SP**(-6.0_SP)*rng%get()
            protein = biomass/float(count)
            carbohydrate = protein * equilibriumRatio(density)
            write(fid, "(5F20.6)") z, radius, carbohydrate, protein, toxin/float(count)
        end do
        close(fid)
    end subroutine
end module
